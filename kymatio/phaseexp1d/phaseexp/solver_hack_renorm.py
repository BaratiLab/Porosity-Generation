...
class SolverHack(nn.Module):
    def __init__(self, embedding, x0, loss_fn, cuda=False):
        ...
        # self.loss_scat0, self.loss_harm0 =
        # self.loss.declare_renorm(self.loss_scat0, self.loss_harm0)
        self.loss.declare_renorm(*self.loss(self.emb0, None))
        self.loss_scat0 = torch.mean(self.loss.loss_scat0)
        self.loss_harm0 = torch.mean(self.loss.loss_harm0)
...
class MSELoss(nn.Module):
    def __init__(self, phi, alpha=0.5, use_cuda=False, eps=1e-16):
        ...
        self.loss_scat = None
        self.loss_harm = None
        self.loss_scat0 = None
        self.loss_harm0 = None

    def declare_renorm(self, loss_scat0, loss_harm0):
        self.loss_scat0 = loss_scat0
        self.loss_harm0 = loss_harm0
    ...
    def forward(self, input, target):
        ...
        if self.loss_scat0 is None:
            loss_scat = self.mse_norm(gap_scat)
        else:
            loss_scat = self.mse_norm(gap_scat, self.loss_scat0 + self.eps)
        if self.loss_harm0 is None:
            loss_harm = self.mse_norm(gap_phe)
        else:
            loss_harm = self.mse_norm(gap_phe, self.loss_harm0 + self.eps)
    ...
    @staticmethod
    def mse_norm(x, renorm=None):
        sq_err = (x ** 2).sum(-1)
        # Hack: mse_norm is called with renorm=None the first time, with the target x.
        # In that case we want to return the tensor of coefficients of x.
        if renorm is None:
            return sq_err.view(-1)
        else:
            return torch.mean(sq_err.view(-1) / renorm)
