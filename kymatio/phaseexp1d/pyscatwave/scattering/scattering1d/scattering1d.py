...
class Scattering1D(object):
    def __init__(self, T, J, Q, normalize='l1', criterion_amplitude=1e-3,
                 ...
                 P_max=5, eps=1e-7, order2=True, average_U1=True, average_U2=True,
                 oversampling=0, vectorize=True, precision='single'):
        """
        ...
        if precision == 'single':
            ...
            self.CudaTensor = torch.cuda.FloatTensor
        elif precision == 'double':
            self.Tensor = torch.DoubleTensor
            self.CudaTensor = torch.cuda.DoubleTensor
        else:
    ...
    def build(self):
        ...
        cast_psi(self.psi1_fft, self.Tensor)
        self.psi2_fft = psi2_fft
        cast_psi(self.psi2_fft, self.Tensor)
        self.phi_fft = phi_fft
        cast_phi(self.phi_fft, self.Tensor)
    ...
    def cpu(self):
        ...
        """
        return self._type(self.Tensor)

    def cuda(self):
        """
        Moves the parameters of the scattering to the GPU
        """
        return self._type(self.CudaTensor)

    def set_default_args(self, order2=True, average_U1=True,
                         average_U2=True, oversampling=0, vectorize=True ):
        """
    ...
    def _get_arguments(self, order2, average_U1, average_U2, oversampling, vectorize):
        new_o2 = self.default_args['order2'] if order2 is None else order2
        new_aU1 = self.default_args['average_U1'] if average_U1 is None\
            else average_U1
        new_aU2 = self.default_args['average_U2'] if average_U2 is None\
            else average_U2
        new_os = self.default_args['oversampling'] if oversampling is None\
            else oversampling
        new_vect = self.default_args['vectorize'] if vectorize is None\
            else vectorize
        return new_o2, new_aU1, new_aU2, new_os, new_vect

    def forward(self, x, order2=None, average_U1=None, average_U2=None,
                oversampling=None, vectorize=None):
        """
        ...
        order2, average_U1, average_U2, oversampling, vectorize = self._get_arguments(
            order2, average_U1, average_U2, oversampling, vectorize)
        # treat the arguments
        if vectorize:
...
def scattering(x, psi1, psi2, phi, J, pad_left=0, pad_right=0,
               ...
               order2=False, average_U1=True, average_U2=True,
               size_scat=0, vectorize=False):
    """
            ...
            for (j2, n2) in sorted(psi2.keys()):
                if j2 > j:
                    ...
                    if average_U2:
                        ...
                        S2_J = unpad(real(ifft1d_c2c_normed(U2_hat)),
                                     ind_start[k1 + k2], ind_end[k1 + k2])
                    if vectorize:
                        S[:, cc] = S2_J
                        cc += 1
                    else:
