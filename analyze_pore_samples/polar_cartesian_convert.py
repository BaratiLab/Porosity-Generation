...
def map_pixel(i,j, img, o=None, r=None, output=None, order=1, cont=0, debug = False, out_h = None, out_w = None):
    ...
    # breakpoint()
def unmap_pixel(radius,theta_idx, img,theta = None, o=None, r=None, output=None, order=1, cont=0, debug = False, out_h = None, out_w = None):
    if r is None: r = img.shape[0]
    if output is None:
        output = np.zeros((r*2, r*2), dtype=img.dtype)
    elif isinstance(output, tuple):
...
if __name__ == '__main__':
    ...
    oldtime = time.time()
    coords = (120,1)
    test =map_pixel(coords[0], coords[1],img[255:,:], o = o, r =r, out_h = out_h, out_w = out_w, debug= False )
    original_args = unmap_pixel(test[0], test[1], out, output = img.shape, theta = test[2])
    print(coords, original_args)
    newtime = time.time()
